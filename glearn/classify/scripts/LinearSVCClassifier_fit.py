...
def LinearSVCClassifier_fit(multilevel_code,categories_select,ratio):
	...
	if not os.path.exists(goal):
		os.mkdir(goal)
	file_name = os.path.join(gv.models_path,multilevel_code)+'/'+'LinearSVCClassifier@' + '_'.join(data_train.target_names)+'.joblib'
	print file_name
	print 
...
def main():
	...
	group_list.append('');print "group_list.append('')"
	for base_code in group_list:
		multilevel_code = base_code
		categories_select = None
		ratio = 1.0
		LinearSVCClassifier_fit(multilevel_code,categories_select,ratio)		
